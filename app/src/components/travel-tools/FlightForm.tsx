...
const OUTBOUND_URL = process.env.NEXT_PUBLIC_FLIGHT_OUTBOUND_URL;
...
export default function FlightForm() {
  ...
  const [summary, setSummary] = useState<{ country: string; region: string; departDate: string; returnDate: string } | null>(null);
  ...
  function handleSubmit(event: React.FormEvent) {
    ...
    setError(null);
    setSummary({ country, region, departDate, returnDate });
  }

  function handleOutbound() {
    if (!OUTBOUND_URL) {
      setError("항공편 검색 서비스 연결에 실패했습니다. 잠시 후 다시 시도해 주세요.");
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <label className="flex flex-col gap-1">
          <span className="text-[14px] leading-[1.4] text-[#6E6E75]">출발일</span>
          <input
            type="date"
            value={departDate}
            min={todayIso()}
        ...
        <label className="flex flex-col gap-1">
          <span className="text-[14px] leading-[1.4] text-[#6E6E75]">귀국일</span>
          <input
            type="date"
            value={returnDate}
      ...
      {summary ? (
        <div className="rounded-[14px] border border-[#E3E1DC] bg-[#F7F6F3] p-4">
          <p className="text-base leading-[1.6] text-[#2A2A2E]">
            {summary.country} {summary.region} · {summary.departDate} ~ {summary.returnDate}
          </p>
