# API-REPORT-BLOCK - 신고·차단·해제

**Task List Seq:** 15  |  **Category:** API  |  **Priority:** P1  |  **Implementation Status:** IMPLEMENT

## Context

SCR-004, SCR-005 화면이 사용하는 Server Action/데이터 접근 로직을 담당한다. 클라이언트 Component는 이 API를 통해서만 쓰기/조회를 수행한다. 관련 Requirement: REQ-FUNC-039, REQ-FUNC-040, REQ-NF-019.
...
- `app/src/lib/actions/reportBlock.ts`(create)

**이 목록 밖의 파일은 수정하지 않는다.**

## Functional AC

- 신고 접수 시 접수번호 즉시 반환(p95 3초 이내 응답), 차단 생성/해제, 차단 후 상호 글·요청 비노출
...
- 신고자/피신고자 상세는 Moderator/Admin만 조회

## Test Cases

- 주어진 상태에서 다음 조건이 성립하는지 확인한다: 신고 접수 시 접수번호 즉시 반환(p95 3초 이내 응답), 차단 생성/해제, 차단 후 상호 글·요청 비노출

## Verify

Playwright: 신고 접수·차단 후 비노출 확인

## Definition of Done

- Functional AC, Visual AC, Security/Privacy AC 전 항목이 충족된다.
- Expected Files에 나열된 파일만 생성/수정되었고, 그 밖의 파일은 변경되지 않았다.
- Verify에 명시된 방법(Playwright: 신고 접수·차단 후 비노출 확인)으로 실제 확인을 마쳤다.
