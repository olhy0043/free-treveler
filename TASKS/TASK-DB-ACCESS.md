# DB-ACCESS - 타입 안전 DB Access Layer

**Task List Seq:** 9  |  **Category:** DB  |  **Priority:** P0  |  **Implementation Status:** IMPLEMENT

## Context

Supabase PostgreSQL 스키마, 정책, 접근 계층 중 한 부분을 담당한다. 전체 DB는 정확히 6개 테이블로 제한되며, 이 Task는 그 범위 안에서만 동작한다. 관련 Requirement: REQ-NF-015.

## Project Scope

docs/PROJECT_SCOPE.md에서 이 Task가 다루는 모든 Requirement(REQ-NF-015)는 IMPLEMENT 계열로 분류되어 있다(IMPLEMENT / IMPLEMENT(부분) / IMPLEMENT(축소) 포함, EXCLUDED 없음). Implementation Status: IMPLEMENT.
...
- `app/src/lib/db/types.ts`(create)

**이 목록 밖의 파일은 수정하지 않는다.**

## Functional AC

- Supabase 타입 생성 반영, 모든 쿼리가 파라미터 바인딩 사용(문자열 결합 금지)
...
- 저장 XSS 방지를 위한 입력 이스케이프/검증 유틸 포함

## Test Cases

- 주어진 상태에서 다음 조건이 성립하는지 확인한다: Supabase 타입 생성 반영, 모든 쿼리가 파라미터 바인딩 사용(문자열 결합 금지)

## Verify

단위 테스트: 악성 입력 문자열로 쿼리 안전성 확인

## Definition of Done

- Functional AC, Visual AC, Security/Privacy AC 전 항목이 충족된다.
- Expected Files에 나열된 파일만 생성/수정되었고, 그 밖의 파일은 변경되지 않았다.
- Verify에 명시된 방법(단위 테스트: 악성 입력 문자열로 쿼리 안전성 확인)으로 실제 확인을 마쳤다.
